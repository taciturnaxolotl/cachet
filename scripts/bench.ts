...
async function main() {
	...
	for (let i = 0; i < 100; i++) {
		...
		await fetch(`${BASE}/users/${userIds[i % userIds.length]}`).then((r) =>
			r.arrayBuffer(),
		);
		await fetch(`${BASE}/emojis/${emojiNames[i % emojiNames.length]}`).then(
			(r) => r.arrayBuffer(),
		);
		await fetch(`${BASE}/users/${userIds[i % userIds.length]}/r`).then((r) =>
			r.arrayBuffer(),
		);
		await fetch(`${BASE}/emojis/${emojiNames[i % emojiNames.length]}/r`).then(
			(r) => r.arrayBuffer(),
		);
	}
	...
	results.push(
		await benchmark(
			"GET /users/:id (data)",
			(i) => `${BASE}/users/${userIds[i % userIds.length]}`,
			DURATION,
			CONCURRENCY,
		),
	);
	...
	results.push(
		await benchmark(
			"GET /users/:id/r (redirect)",
			(i) => `${BASE}/users/${userIds[i % userIds.length]}/r`,
			DURATION,
			CONCURRENCY,
		),
	);
	...
	results.push(
		await benchmark(
			"GET /emojis/:name (data)",
			(i) => `${BASE}/emojis/${emojiNames[i % emojiNames.length]}`,
			DURATION,
			CONCURRENCY,
		),
	);
	...
	results.push(
		await benchmark(
			"GET /emojis/:name/r (redirect)",
			(i) => `${BASE}/emojis/${emojiNames[i % emojiNames.length]}/r`,
			DURATION,
			CONCURRENCY,
		),
	);
	...
	results.push(
		await benchmark(
			"GET /api/stats/essential",
			() => `${BASE}/api/stats/essential`,
			DURATION,
			CONCURRENCY,
		),
	);
	...
	results.push(
		await benchmark(
			...
			(i) => {
				...
				if (mod < 5)
					return `${BASE}/emojis/${emojiNames[i % emojiNames.length]}/r`;
				if (mod < 7) return `${BASE}/users/${userIds[i % userIds.length]}`;
				if (mod < 9)
					return `${BASE}/emojis/${emojiNames[i % emojiNames.length]}`;
				return `${BASE}/health`;
			},
			DURATION,
			CONCURRENCY,
