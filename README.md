...
> [!NOTE]
> The map now regenerates itself. Run `bun run scripts/build-emoji-map.ts [version]` and it rebuilds [`utils/slack_emoji_map.json`](./utils/slack_emoji_map.json) from [iamcal/emoji-data](https://github.com/iamcal/emoji-data), which is the dataset Slack's own emoji picker is built on. The dataset version tracks the cdn asset version, so `emoji-datasource@16.0.0` lines up with `production-standard-emoji-assets/16.0/`; the script derives the asset version and writes it into the map, so there's only one version to bump.
>
> The old manual route, for posterity: open the react dev tools to the component tab and copy the `consolidatedEmojis` field, in a brand new workspace so no custom emojis get lumped in.
...
{
  "grinning": "https://a.slack-edge.com/production-standard-emoji-assets/16.0/google-xlarge/1f601.png"
}
```
