...
/**
 * The same suite runs against every configured backend.
 *
 * SQLite always runs. Postgres runs when TEST_DATABASE_URL points at a
	...
	describe(`SlackCache integration (${backend.name})`, () => {
		...
		describe("user CRUD", () => {
			...
			it("updates all fields on conflict", async () => {
				...
				expect(user?.displayName).toBe("New Display");
				expect(user?.realName).toBe("New Real");
		...
		describe("uptime tracking", () => {
			it("reports uptime percentage", async () => {
				const uptime = await cache.getUptime();
		...
		describe("analytics recording", () => {
			...
			it("retrieves traffic data", async () => {
